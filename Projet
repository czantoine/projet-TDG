...
{ 
    ...
    Graph(int v, int arc) //constructeur du graphe vide
    { 
        Sommet = v;
        this.arcs= arc;
	...
	void addEdge(Integer depart,Integer arrivee, int poids) //initialisation d'une arrete
	...
	void valeur_tab()		// affichage Matrice des valeurs
	{
	...
	void adjacence_tab() // affichage tableau d'adjacence 
	{   
    ...
    void affiche_rang(ArrayList<Integer> rang) //affiche les rangs par sommet
    ...
    void affiche_plustot(ArrayList<Integer> ord) // affichage de la date au plus tot par sommet
    {
    ...
    void affiche_plustard(ArrayList<Integer> ord) // affichage de la date au plus tard par sommet
    {
    ...
    void affiche_marge(ArrayList<Integer> ord) // affiche la marge
    {
	...
	void affiche_graph() //affichage du graphe
	{
    ...
    ArrayList<Integer> jinvoquelesprede(Integer sommet)// fonction permettant de trouver tous les predecesseurs d'un sommet
    {	
    ...
    ArrayList<Integer> jinvoquelessucc(Integer sommet) //fonction permettant de trouver tous les successeurs d'un sommet
    {	
    	ArrayList<Integer>  aide = new ArrayList<>(); 
    ...
    ArrayList<Integer> Def_my_rank() //defini les rangs de maniere iterative 
    {
    	...
    	while(liste_rang.contains(null)  ) // tous les sommets soivent avoir un rang
    	{
    		
    		for(int i = 0 ; i< graphe.length; i++)
    		{
    			if(liste_rang.get(i) !=null ) // si le range est dja connu on passe
    			{
    				continue;
    			}
    			...
    			liste_rang.set(i, max_rang +1); //On définit les rangs on fonction des sommets du rang precedent 
    ...
    private boolean isCyclicUtil(int i, boolean[] deja_visite, // fonction recursive permettant de trouver les circuits 
    ...
    ArrayList<Integer> Liste_entree()// retourne la liste des entrées du graphes
    {
    	ArrayList<Integer>  liste_entree = new ArrayList<>();    
    	for(int i = 0; i < graphe.length; i++)
    	{
    		
	    	var pred = jinvoquelesprede(i); //Si le sommet n'a pas de predecesseurs alors il est entrée 
	    	if(pred.isEmpty()) 
	    	{
	    		liste_entree.add(i);
	    	}
    ...
    ArrayList<Integer> Liste_sortie()//// retourne la liste des sorties du graphes
    {
    	ArrayList<Integer>  liste_sortie = new ArrayList<>(); 
    	for(int i = 0; i < graphe.length; i++)
    	{
    		
	    	var pred = jinvoquelessucc(i);//Si le sommet n'a pas de successeurs alors il est sortie
	    	if(pred.isEmpty()) 
	    	{
	    		liste_sortie.add(i);
	    	}
    ...
    { 
		...
		ord.set(entree, 0); //entrée rang 0 obligatoirement 
		var succ = jinvoquelessucc(entree);   //on commence le calcul de la date au plus tot a partir des successeurs de l'entrée
    ...
    void plutotbis(Integer Sommet, ArrayList<Integer> ord)  //boucle recursive pour trouver les dates aux plus tot de chaque sommet 
    {
    	...
    	{
    		if(ord.get(pred)==null || ord.get(Sommet) != null ) //condition de sortie
    		{
    			return;
    		}
    	...
    	for(Integer pred : preds ) // on regarde chaque predecesseurs
    	{
    		var curr = ord.get(pred) + graphe[pred][Sommet];
    		
    		if (max_ord < curr)//et on prend la valeur max d'arrete
    		{
    			max_ord = curr;   		
    		}
    	}
    	...
    	for(Integer succ : succs )//et on relance la fonction recursive sur les successeurs du sommet etudié
    	{
    		plutotbis(succ,ord);
    	}
   ...
   ArrayList<Integer> plutardcall(Integer plustotlast)  // principe similaire a date au plus tot
    ...
    ArrayList<Integer> marge_ord(ArrayList<Integer> plus_tot,ArrayList<Integer> plus_tard)//difference entre la date au plus tot et celle au plus tard pour chaque sommet
    {
    ...
    Integer sum_succ(Integer i)// calcul la somme poids vers les successeurs
    {
    	Integer Sum= 0 ;
    ...
    boolean val_id()//verifie si toutes les arcs qui sortent d'un sommet on une valeur identique
    {
    ...
    boolean neg_arc() // vérification si il y a des valeurs négatives
    {
    	for(int i = 0 ; i< graphe.length;i++)
    	{
    		for(int j = 0 ; j< graphe.length;j++)
     ...
     boolean Ordonnancement() //vérifications des conditions d'ordonnancement
    {
    ...
    { 
       ...
       do { 
		...
		String path = "src\\graphes\\" + fichier +".txt"; //path pour trouver le graphe
